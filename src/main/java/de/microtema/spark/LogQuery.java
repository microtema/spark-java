...
public final class LogQuery {
    ...
    public static void main(String[] args) {
        ...
        try (var jsc = new JavaSparkContext(spark.sparkContext())) {
            ...
            var dataSet = jsc.parallelize(exampleApacheLogs);

            var extracted = dataSet.mapToPair(s -> new Tuple2<>(extractKey(s), extractStats(s)));

            var counts = extracted.reduceByKey(Stats::merge);

            counts.collect().forEach(it -> System.out.println(it._1() + "\t" + it._2()));
        }
    }
}